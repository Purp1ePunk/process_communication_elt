...
char pathname[] = "8 first.c"; /* Имя файла, использующееся для генерации ключа. Файл с таким именем должен существовать в текущей директории */
key_t key; /* IPC ключ */
...
mybuf.sem_op = 1; //A(s1, 1)
mybuf.sem_flg = 0;
mybuf.sem_num = 0; //Нулевой элемент массива структур
...
if((array = (int *)shmat(shmid, NULL, 0)) == (int *)(-1)){ //A(S1, 1)
